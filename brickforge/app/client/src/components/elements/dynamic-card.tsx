        ...
        <div className="border-t border-inherit px-4 py-2">
          {fields.map((f, i) => (
            <div key={i} className="flex justify-between py-1 text-xs">
              ...
              <span className="font-medium text-gray-800 dark:text-gray-200">{f.value}</span>
            </div>
          ))}
        </div>
      )}
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-inherit">
                  {columns.map((col, i) => (
                    <th key={i} className="px-3 py-1.5 text-left font-medium text-gray-500 dark:text-gray-400">
                      {col}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {rows.map((row, i) => (
                  <tr key={i} className="border-b border-inherit last:border-b-0">
                    {row.map((cell, j) => (
                      <td key={j} className="px-3 py-1.5 text-gray-800 dark:text-gray-200">
                        {cell ?? ''}
                      </td>
                    ))}
                  </tr>
          ...
          {!confirmed && sendMessage && (
            <div className="px-4 py-2.5 border-t border-inherit">
              <button
                ...
                className="w-full px-3 py-1.5 rounded-lg bg-gray-800 dark:bg-gray-200 text-white dark:text-gray-800 text-xs font-medium hover:opacity-90 transition-opacity"
              >
                Confirm
              </button>
            </div>
