...
You want to build an AI agent on Databricks. You've got all the building blocks. Unity Catalog, Genie, Knowledge Assistants, Foundation Models, Apps. BrickForge wires them all together for you. All you need is a pip install:
...
Databricks has everything you need to build an agent. Unity Catalog, SQL functions, stored procedures, model endpoints, Genie spaces, Knowledge Assistants, Apps. It's all there. BrickForge brings it all into one place and walks you through the setup.
...
Pick your Foundation Model. Auto-detect scans your workspace for every available serving endpoint. Claude, Llama, Mixtral, DBRX - anything served via Foundation Model APIs works. Pick whichever model fits your use case. *Click to select.*
...
Now the interesting wiring. Genie gives your agent natural-language SQL -it asks Genie questions, Genie writes the SQL, queries your tables, returns results. Genie handles the SQL for you.
...
You own everything. And later, we'll push this whole agent to GitHub - but let's keep going.
...
That's the philosophy: BrickForge is the scaffolding. It gets you there fast. Then it's your codebase, your workflow, your rules.
...
All config lives in a single config.json. Clean and portable. The agent reads it at boot, flattens to env vars, and every tool discovers itself.
